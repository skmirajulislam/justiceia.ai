...
const Footer = () => {
  const footerLinks = {
    Platform: [
      ...
      { name: 'Document Processor', href: '/document-processor' }
    ],
    Legal: [
    ...
    <footer className="bg-slate-900 text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-8">
          ...
          {/* Links sections */}
          <div className="lg:col-span-4 grid [grid-template-columns:repeat(auto-fit,minmax(120px,1fr))] gap-6">
            {Object.entries(footerLinks).map(([category, links]) => (
              <div key={category}>
                ...
                <ul className="space-y-2">
                  {links.map((link) => (
                    <li key={link.name}>
                      <a href={link.href} className="text-slate-400 hover:text-sky-400 transition-colors duration-200 text-sm">
                        {link.name}
                      </a>
                    </li>
                  ))}
          ...
          <div className="flex">
            ...
            {/* <div className="flex space-x-4">
              <input
                ...
                className="flex-1 px-3 py-2 text-sm bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 text-white placeholder-slate-400"
              />
              <button className="px-4 py-2 text-sm bg-gradient-to-r from-sky-400 to-sky-500 hover:from-sky-500 hover:to-sky-600 text-white rounded-lg font-medium transition-all duration-200 whitespace-nowrap">
                Subscribe
              </button>
            </div> */}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-left">
            <div className="text-slate-400 text-sm mb-4 md:mb-0">
              © 2025 Justiceia.ai. All rights reserved. | Built with ❤️ for the Indian Legal Community
            </div>
            {/* Social links */}
            <div className="flex space-x-4">
